...
if(isset($_POST['submit'])){
	...
	$prix = $_POST['prix'].' '.'€' ; 
	 ...
	 $path = 'uploads';
        ...
        $req = $bdd->prepare('UPDATE products SET clé = ? , marque = ? , prix = ? , photo = ? WHERE id = ?') ; 
        $succ = $req->execute([$key,$marque,$prix,$filename,$id]) ; 
        if($succ){
        	$msg = 'clé modifier' ;
	...
	<header>
		<div class="head_top">
			...
			<div class="email">
				...
				<a href="mailto:#">djameltliba@gmail.com/a>
			</div>
...
<input style="position:relative; left:24%" id="sub" type="submit" name="submit" value="Modifier">
